...
class MessageHandlerImpl(MessageHandler):
    ...
    def on_message(self, message: InboundMessage):
        ...
        print(f"\n[ETH] Received message on: {topic}")
        print(f"[ETH] Payload: {payload}\n")
...
print(f'ETH Subscriber connected? {messaging_service.is_connected}')
...
try:
    ...
    print(f'[ETH Subscriber] Bound to Queue [{durable_exclusive_queue.get_name()}]')

    while True:
        time.sleep(1)

except PubSubPlusClientError:
    print(f'\nMake sure queue {queue_name} exists with topic subscription: crypto/ethereum')
